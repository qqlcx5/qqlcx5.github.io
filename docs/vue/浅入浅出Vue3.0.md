...
在一次意外的看到Vue3.0课程打折，而且还是个大牛在教，我被贪图小利的驱使下，购买了这个课程，然后想着课程买了不看浪费。图了自己良心过意得去进去看看，然后就被被`Composition API`等写法震撼到了。在未接触`Vue3.0`之前我听说`Vue3.0`模仿 `React`等其他语言，很多东西大改，改的不像Vue，我在想那为啥不学 React？等言论纠结的一段时间，最后选择去学学`React`看看。原因`React`用户基数多，大型项目完善，找工作也更容易等。在没规划的学习`React`一段时间后，感觉自己学到一些皮毛，而且学的很慢，可能方法没找对。 （后续过段时间境界提升了再去学习react）

## 反思

被`Vue3.0`等新的语法震撼到后，我去看`Composition API`官网。得到一些信息后。决定认真去学`Vue3.0`，在学了一两个小时后，发现`Vue3.0`比想象中更加简单。网上这方面的教程估计很少，很多是介绍`Vue3.0`新特性这类的吧。在下觉得学会`Composition API` 后用Vue3.0项目实战入手没什么问题。


## Vue3.0

从尤大大直播中了解到，Vue3.0改动挺大的，估计Vue基本重写吧。而且还兼容Vue2.0所有API。牛逼牛逼
...
接下来，介绍Vue3.0和Vue2.0之间的差别和经验之谈，菜鸟一枚，目前失业，大言不惭各位面前献丑了。熟悉以下知识点估计面试、项目实战木问题。
...
7月份左右正式版就正式发布，`Vue Composition API`基本上是板上钉钉的事，几乎不会在做调整，现在学好等正式版出来直接使用岂不美哉。
