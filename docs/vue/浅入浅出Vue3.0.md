...
被`Vue3.0`等新的语法震撼到后，我去看`Composition API`官网。决定认真去学`Vue3.0`，在学了一两个小时后，发现`Vue3.0`比想象中更加简单。网上这方面的教程估计也很少，很多是介绍`Vue3.0`新特性这类的吧。Vue3.0项目实战主要以`Composition API`为主，学会了估计Vue3.0也就差不多了。
...
8月份左右正式版就正式发布了，`Vue Composition API`基本上是板上钉钉的事，几乎不会在做调整，现在学好等正式版出来直接使用岂不美哉。
