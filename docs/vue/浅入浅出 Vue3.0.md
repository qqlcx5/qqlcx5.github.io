...
昨天晚上碰巧看到 Vue3.0相关的视频，顺便进去看看，被`Composition API`等写法震撼到了。在未接触`Vue3.0`之前我听说`Vue3.0`模仿 `React`等其他语言，很多东西大改，改的不像Vue，我在想那为啥不学 React？而花时间去学习`Vue3.0`，等言论纠结的一段时间，最后选择去学学`React`看看。原因`React`用户基数更大，找工作也更加容易等。在没规划的学习`React`一段时间后感觉自己学到一些皮毛，而且学的很慢，可能方法没找对。 （后续过段时间境界提升了再去学习react）

## 反思

被`Vue3.0`等新的语法震撼到后，我去看`Composition API`官网。得到一些信息后。决定认真去学`Vue3.0`，在学了一两个小时后，发现`Vue3.0`比想象中更加简单。网上这方面的教程估计很少，很多是介绍`Vue3.0`新特性这类的吧。在下觉得学会`Composition API` Vue3.0项目实战问题不大。
